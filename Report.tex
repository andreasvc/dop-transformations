...
The corpus we evaluate on consists of 187 exemplars of declarative sentences
paired with interrogative sentences. Half of these were interrogative sentences
found in the Wall Street Journal (WSJ) corpus, and the other half declarative
sentences from the same corpus. These sentences were then transformed by hand,
and fed to the Stanford parser to obtain parse trees. The resulting parse trees
were manually `corrected' so as to conform to the corresponding tree from the
WSJ. The corpus has 33 unique S productions, and 91 unique SQ productions.

Evaluation is done on lemmatized versions of the resulting surface forms, and
contractions are expanded.  Words and POS tags of the test set were added to the
...
\begin{tabular}{ r l l }
\hline\hline
				& DOT	  & partial subtrees \\
decl $\rightarrow$ inter 	& 27 \%	  & 40.5 \% (62.5 \% if lower ranking matches are counted) \\
inter $\rightarrow$ decl 	& 21.5 \% & 47 \% (69.5 \% if lower ranking matches are counted) \\
\hline
\end{tabular}
